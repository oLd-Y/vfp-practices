...
6. 打开时，速度较慢，算法可以改进吗？同时要给用户以提示，不然他们要认为死机了。注意人机交互的方式要友好，最好是动态的，让用户看到系统在工作。用户的电脑比你们要慢十倍不止，不能以你的电脑本地独占运行为标准，要以多用户、差电脑、满网络为标准。这点以后不再说明
...
除了修正上述问题外，加做练习4：
在练习3的FORM基础上，加一个按钮，弹出1个子FORM：

1. 目的是让操作者（招生办工作人员）选择不同的大学后，显示该大学的被投档学生的信息GRID（内容和次序同练习3的GRID内容）和最低投档线的总分和排位，投档的规则是：
    1) 最多是该大学计划录取人数的1.3倍
       2）语数外三科成绩总分不低于280分
       3）先满足第一志愿是该大学的，人数不足1.3倍时，再依次考虑第二志愿、第三志愿
       4）如果人数依然不足1.3倍，则结束
       2、按显示内容打印成报表
       3、可以方便地按姓名、中学校名和身份证过滤被投档学生信息GRID
       4、连同练习3的顶层表单和子表单一起建立一个项目，项目图标任意选择，编译成EXE
